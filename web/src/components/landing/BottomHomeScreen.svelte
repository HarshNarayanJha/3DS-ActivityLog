...
import BottomScreen from "./BottomScreen.svelte"
...
let { children, isLoading = false, openEnabled = false, onOpenClicked }: Props = $props()

const appletIcons = [
  { path: asset("/icons/game_notes.png") },
  { path: asset("/icons/friends.png") },
  { path: asset("/icons/notifications.png") },
  { path: asset("/icons/browser.png") },
  { path: asset("/icons/miiverse.png") }
]
...
<BottomScreen>
  {#if isLoading}
    <div class="aspect-[1.38] h-full w-full bg-black" in:fade={{ duration: 500 }}></div>
  {:else}
    <div class="mx-auto aspect-[1.38] h-auto w-full bg-gray-100">
      <div class="grid-row-3 grid h-full w-full grid-cols-1">
        ...
        <div
          ...
          <button
            class="rounded-tr-2xl border border-b-0 border-neutral-500 bg-linear-to-b from-neutral-200 from-50% to-neutral-400 px-4 py-1 active:opacity-75 disabled:opacity-50"
